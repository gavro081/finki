...
if __name__ == '__main__':
    ...
    # good = [row for row in dataset if row[-1] == 0]
    # bad = [row for row in dataset if row[-1] == 1]

    # train_data = good[:int(0.8 * len(good))] + bad[:int(0.8 * len(bad))]
    train_data = dataset[:int(len(dataset)*0.8)]
    train_x = [row[:-1] for row in train_data]
    train_y = [row[-1] for row in train_data]

    # test_data = good[int(0.8 * len(good)):] + bad[int(0.8 * len(bad)):]
    ...
    predictions_train = model.predict(scaler.transform(train_x))
    predictions_test = model.predict(scaler.transform(test_x))
    # train_score = model.score(scaler.transform(train_x), train_y)
    # test_score = model.score(scaler.transform(test_x), test_y)
    test_score, train_score = 0, 0
    for real, pred in zip(train_y, predictions_train):
        if real == pred:
            train_score +=1

    train_score /= len(train_y)

    for real, pred in zip(test_y, predictions_test):
        if real == pred:
            test_score +=1

    test_score /= len(test_y)

    # print(train_score)
    # print(test_score)

    if test_score * 1.15 < train_score: # overfit
        train_data = [row[:col] + row[col + 1:] for row in train_data]

        train_x = [row[:-1] for row in train_data]
        train_y = [row[-1] for row in train_data]
        scaler.fit(train_x)
        query = query[:col] + query[(col + 1):]
