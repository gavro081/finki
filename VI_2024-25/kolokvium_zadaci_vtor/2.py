...
if __name__ == '__main__':
    ...
    training_data = dataset[:int(len(dataset) * 0.85)]
    testing_data = dataset[int(len(dataset) * 0.85):]

    train_x = [row[:-1] for row in training_data]
    train_y = [int(row[-1]) for row in training_data]

    test_x = [row[:-1] for row in testing_data]
    test_y = [int(row[-1]) for row in testing_data]
