...
if __name__ == '__main__':
    ...
    bayes_data = c0[:int(len(c0) * x1)] + c1[:int(len(c1) * x1)] + c2[:int(len(c2) * x1)]
    decision_tree_data = c0[int(len(c0) * x1):int(len(c0) * x2)] + c1[int(len(c1) * x1):int(len(c1) * x2)] + \
                         c2[int(len(c2) * x1):int(len(c2) * x2)]
    forrest_data = c0[:int(len(c0) * x2)] + c1[:int(len(c1) * x2)] + c2[:int(len(c2) * x2)]
    test_data =  c0[int(len(c0) * x2):] + c1[int(len(c1) * x2):] + c2[int(len(c2) * x2):]


    bayes_x = [row[:-1] for row in bayes_data]
    bayes_y = [row[-1] for row in bayes_data]

    decision_tree_x = [row[:-1] for row in decision_tree_data]
    decision_tree_y = [row[-1] for row in decision_tree_data]

    forrest_x = [row[:-1] for row in forrest_data]
    forrest_y = [row[-1] for row in forrest_data]
    ...
    forrest = RandomForestClassifier(n_estimators=3, random_state=0)

    bayes.fit(bayes_x, bayes_y)
    decision_tree.fit(decision_tree_x, decision_tree_y)
    forrest.fit(forrest_x, forrest_y)

    pred1 = bayes.predict(test_x)
    pred2 = decision_tree.predict(test_x)
    pred3 = forrest.predict(test_x)

    total_count = 0
    for p1,p2,p3,real in zip(pred1, pred2, pred3, test_y):
        count = 0
        for p in [p1,p2,p3]:
            if p == real: count += 1
            if count >= 2:
                total_count += 1
                break
    print(f"Tochnost: {total_count / len(test_y)}")
