...
from searching_framework import depth_first_graph_search
...
class GhostOnSkates(Problem):
    ...
    def h(self, node):
        # {'state': (3, 3), 'parent': <Node (3, 2)>, 'action': 'Gore 1',
        # 'path_cost': 3, 'depth': 3}
        # print(vars(node['state']))
        state = node.state
        # return math.sqrt((n - state[0]) ** 2 + (n - state[1]) ** 2)
