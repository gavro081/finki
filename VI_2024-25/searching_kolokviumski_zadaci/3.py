...
class Towers(Problem):
    ...
    def successor(self, state):
        ...
        top_blocks = [pillar.split(',')[-1] if pillar else '#' for pillar in curr_state]
        for i, block in enumerate(top_blocks):
            if block == '#':
                continue
            for j in range(len(top_blocks)):
                if i != j and (top_blocks[j] == '#' or int(top_blocks[j]) >= int(block)):
                    new_state = curr_state[:]
                    new_state[i] = ",".join(map(str,new_state[i].split(",")[:-1]))
                    if new_state[j] == '':
                        new_state[j] = block
                    else:
                        tmp = new_state[j].split(",")
                        tmp.append(block)
                        new_state[j] = ",".join(map(str, tmp))
                        # new_state[j] = ",".join(map(str, new_state[j].split(",").append(block)))
...
if __name__ == "__main__":
    ...
    problem = Towers(tuple(initial_state), tuple(goal_state))
    solution = breadth_first_graph_search(problem)
    print("Number of action",solution.depth)
