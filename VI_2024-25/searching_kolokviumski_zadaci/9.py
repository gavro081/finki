# ne proagaat site testovi, pojma neam zaso, ama ist e brojot na cekori
import bisect
...
class SnakeProblem(Problem):
    ...
    def h(self, node):
        state = node.state
        head_x, head_y = state[0][-1]
        if len(state[2]) == 0:
            return 0
        distance = 10000
        for x,y in state[2]:
            distance = min(abs(head_x - x) + abs(head_y - y), distance)
        return distance + len(state[2]) - 1

if __name__ == "__main__":
    n = int(input())
    zeleni_jabolki = [tuple(map(int, input().split(','))) for _ in range(n)]
    snake_body = ((0, 9), (0, 8), (0, 7))
    initial_state = (snake_body, Direction.DOWN.value, tuple(zeleni_jabolki))
    s = SnakeProblem(initial_state)
    solution = astar_search(s)
