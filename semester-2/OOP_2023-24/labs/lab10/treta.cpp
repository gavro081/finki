...
public:
    void print(){
        cout << "Conference duration exceeded\n";
    }
};
...
class Speaker{
    string ime;
    string naslov;
    unsigned int traenje;
public:
    Speaker(string ime="", string naslov="", int traenje=0)
    : ime(ime), naslov(naslov), traenje(traenje){}

    friend ostream& operator<<(ostream& out, Speaker& ob){
        out << ob.ime << " - " << ob.naslov << ", " << ob.traenje;
        return out;
    }

    unsigned int getTraenje(){return traenje;}
};

class Conference{
    string kod;
    string datum;
    vector<Speaker> predavaci;
    static int dozvoleno;
public:
    ...
    friend ostream& operator<<(ostream& out, Conference& ob){
        out << ob.kod << " - " << ob.datum << ", " << ob.predavaci.size();
        return out;
    }

    static void setMaxDuration(int a){
        dozvoleno = a;
    }

    bool operator>(Conference& ob){
        return predavaci.size() > ob.predavaci.size();
    }

    bool operator==(Conference& ob){
        return datum == ob.datum;
    }

    Conference& operator+=(Speaker ob){
        unsigned int vkupno = 0;
        for (int i = 0; i < predavaci.size(); ++i) {
            vkupno += predavaci.at(i).getTraenje();
        }
        try {
            if (vkupno >= dozvoleno) throw InvalidOperation();
        }
        catch (InvalidOperation) {
            InvalidOperation().print();
        }
        predavaci.push_back(ob);
        return *this;
    }
    ...
    void longestPresentation(){
        ...
        for (int i = 0; i < predavaci.size(); ++i) {
            if (predavaci.at(i).getTraenje() > longest) {
                longest = predavaci[i].getTraenje();
                maxi = i;
            }
        }
        cout << "Longest Presentation: " << predavaci[maxi] << '\n';
    }
};
int Conference::dozvoleno = 360;
