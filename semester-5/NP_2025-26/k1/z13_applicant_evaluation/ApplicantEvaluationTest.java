package k1.z13_applicant_evaluation;
...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        // вашиот код овде
        switch (type) {
            case NO_CRIMINAL_RECORD: return x -> !x.hasCriminalRecord();
            case MORE_EXPERIENCE: return x -> x.getEmploymentYears() >= 10;
            case MORE_CREDIT_SCORE: return x -> x.getCreditScore() >= 500;
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return x -> !x.hasCriminalRecord() && x.getEmploymentYears() >= 10;
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                return x -> !x.hasCriminalRecord() && x.getCreditScore() >= 500;
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                return x -> x.getEmploymentYears() >= 10 && x.getCreditScore() >= 500;
            case INVALID:
            default:
