...
class University {
    ...
    private List<Course> getCourses(){
        return departments.stream()
                .map(d -> new ArrayList<>(d.getCourses()))
                .flatMap(List::stream)
    ...
    public List<Course> getCoursesWithMinCredits(int minCredits) {
        return getCourses().stream()
                .filter(d -> d.getCredits() >= minCredits)
                .collect(Collectors.toList());
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        return departments.stream()
                .max(Comparator.comparing(d -> d.getCourses()
                        .stream()
                        .mapToInt(Course::getEnrolledStudents)
                        .sum()
                ));
    ...
    public University mergeFourSmallestDepartments() {
        List<Department> smallest = departments.stream()
                .sorted(Comparator.comparingInt(d -> d.getCourses()
                        .stream()
                        .mapToInt(Course::getEnrolledStudents)
                        .sum()
                ))
        ...
        Department newDepartment = smallest.stream()
                .reduce((d1, d2) -> {
                    String mergedName = d1.getName() + " & " + d2.getName();
        ...
        List<Department> updatedDepartments = new ArrayList<>(this.departments);
        updatedDepartments.removeAll(smallest);
        updatedDepartments.add(newDepartment);
