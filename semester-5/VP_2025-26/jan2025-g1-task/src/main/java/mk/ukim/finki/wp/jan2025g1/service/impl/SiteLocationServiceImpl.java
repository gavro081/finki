...
public class SiteLocationServiceImpl implements SiteLocationService {
    private final SiteLocationRepository repository;

    @Override
    public SiteLocation findById(Long id) {
        return repository.findById(id).orElseThrow(InvalidSiteLocationIdException::new);
    }
    ...
    public SiteLocation create(String city, String country) {
        return repository.save(new SiteLocation(city, country));
    }
}
