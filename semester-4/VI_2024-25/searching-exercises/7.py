...
class Table(Problem):
    def __init__(self, initial, n, goal_state):
    ...
    def successor(self, state):
        successors = {}
        curr_state = list(list(row) for row in state)
        for i in range(self.n):
            for j in range(self.n):
                new_state = [row[:] for row in curr_state]
                for moves in [(-1, 0), (1, 0), (0, -1), (0, 1), (0,0)]:
                    new_i = i + moves[0]
                    new_j = j + moves[1]
                    if 0 <= new_i < self.n and 0 <= new_j < self.n:
                        new_state[new_i][new_j] = 1 if new_state[new_i][new_j] == 0 else 0
                successors[f"x: {i}, y: {j}"] = tuple(tuple(row) for row in new_state)
    ...
    def goal_test(self, state):
        return list(list(row) for row in state) == goal_state


if __name__ == "__main__":
    n = int(input())
    fields = [int(n) for n in input().split(",")]
    table = [fields[i:i+n] for i in range(0, len(fields), n)]
    goal_state = [[1] * n] * n
    initial_state = tuple(tuple(row) for row in table)
    p = Table(initial_state, n, goal_state)
