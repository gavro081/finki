...
if __name__ == '__main__':
    ...
    train_data = good[:int(len(good) * x)] + bad[:int(len(bad) * x)]
    test_data = good[int(len(good) * x):] + bad[int(len(bad) * x):]
    ...
    net = MLPClassifier(10, random_state=0)
    ...
    net_score = net.score(test_x, test_y)

    scores = [bayes_score, tree_score, forest_score, net_score]
    models = [bayes, tree, forest, net]
    labels = ["Naive Bayes", "Decision Tree", "Random Forest", "Neural network"]
    best_i = scores.index(max(scores))
    print(f"Najgolema tocnost ima klasifikatorot {labels[best_i]}")
