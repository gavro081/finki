...
if __name__ == '__main__':
    ...
    training_data = dataset[int(percent * len(dataset)):]
    testing_data = dataset[:int(percent * len(dataset))]
    ...
    model.fit(standard_scaler.transform(train_x), train_y)
    print(f"Tocnost so StandardScaler: {model.score(standard_scaler.transform(test_x), test_y)}")

    min_max_scaler.fit(train_x)
    model.fit(min_max_scaler.transform(train_x), train_y)
    print(f"Tocnost so MinMaxScaler: {model.score(min_max_scaler.transform(test_x), test_y)}")
