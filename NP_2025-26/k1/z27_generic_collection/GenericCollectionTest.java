package k1.z27_generic_collection;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp>{
    ...
    public void addGenericItem (String category, T element){
        itemsMap.putIfAbsent(category, new TreeSet<>());
        itemsMap.get(category).add(element);
        items.add(element);
    }
    ...
    public Map<String, Set<T>> byMonthAndDay(){
        ...
        for (T item : items){
            ...
            ans.putIfAbsent(date, new TreeSet<>(Comparator.reverseOrder()));
            ans.get(date).add(item);
        }
        return ans;
    }
    public Map<Integer, Long> countByYear(){
        ...
        for (T item : items){
            ...
            ans.put(y, ans.getOrDefault(y, 0L) + 1);
        }
        return ans;
    }
}
