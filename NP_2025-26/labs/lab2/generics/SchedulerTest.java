...
class Timestamp<T> implements Comparable<Timestamp<T>> {
    ...
    public int compareTo(Timestamp<T> t){
        return this.localDateTime.compareTo(t.localDateTime);
    }
    ...
    public boolean equals(Object o){
        ...
        Timestamp<T> other = (Timestamp<T>) o;
        return this.localDateTime.equals(other.getLocalDateTime());
    }

    @Override
    public String toString(){
        return localDateTime.toString() + " " + element.toString();
    }
}
...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
         return timestamps.stream()
                .sorted(Comparator.comparing(Timestamp::getLocalDateTime))
                .filter(t -> t.getLocalDateTime().isAfter(LocalDateTime.now()))
                .findFirst()
                .get();
    }
    ...
    public Timestamp<T> last(){
        return timestamps.stream()
                .sorted((a,b) -> b.getLocalDateTime().compareTo(a.getLocalDateTime()))
                .filter(t -> t.getLocalDateTime().isBefore(LocalDateTime.now()))
                .findFirst()
                .get();
    }
    ...
    List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestamps.stream()
                .filter(t -> t.getLocalDateTime().isBefore(end) &&
                        t.getLocalDateTime().isAfter(begin))
